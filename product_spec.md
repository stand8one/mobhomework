...
| 组件 | 形态 | 核心职责 |
|------|------|----------|
| **Android App** | 手机 App（架在高拍仪/支架上） | 作业录入、定时拍照采集、实时预览、本地缓存 |
| **AI Agent** | 云端服务 | 题目解析、进度追踪、效率分析、异常检测 |
| **Web App** | 浏览器端 | 家长查看报告、历史回顾、设置管理 |
...
| 功能项 | 说明 |
|--------|------|
| 定时拍照 | 可配置间隔时间（默认 3 分钟），自动拍照 |
| 智能采集 | （进阶）检测到画面变化较大时才触发拍照，节省资源 |
| 遮挡检测 | 检测到画面被遮挡（如人头挡住）时，标记该帧为"遮挡"，不发送给 Agent 进行进度判断 |
| 模糊检测 | 检测到画面模糊时，标记为"低质量"，Agent 进行降级处理 |
...
> [!IMPORTANT]
> 孩子界面的核心是 **一眼看到「我在哪 vs 我应该在哪」**，产生超越计划的动力。

| 功能项 | 说明 |
|--------|------|
| **进度赛跑条** | 双轨进度条：上方是「预估计划线」（匀速推进），下方是「实际进度线」。孩子能直观看到自己是领先还是落后计划 |
| 当前题目高亮 | 当前正在做的题目 + 已完成数 / 总题数 |
| 领先/落后提示 | 领先计划时显示「你比计划快了 2 题！」；落后时温和提醒「加油，还差 1 题追上计划」 |
| 番茄钟 | 倒计时 + 休息提醒（时长可由家长配置） |
| 小成就弹窗 | 完成一个番茄钟/超越计划时，弹出短暂的庆祝动画 |
| 简洁界面 | 界面极简，核心只有 **进度条 + 番茄钟**，不分散注意力 |
...
| 工作量预估 | 根据题型和数量，预估每道题的完成时间 |
| 总计划生成 | 结合番茄工作法，生成建议的做题顺序和休息节奏 |
...
> [!NOTE]
> 落后阈值应可由家长在 Web App 中配置。默认值基于题目总量的比例动态计算。

---

### 4.3 Web App（家长端 — 省心模式）

> [!IMPORTANT]
> 家长端设计原则：**默认不需要看，有事才通知**。打开时能快速掌握全局，但日常靠推送通知就够了。
...
└────────┴────────┴────────┴────────┴──────────────┘
  ↑                  ↑                  ↑
  每3分钟拍照        每3分钟拍照        每3分钟拍照
```

- 番茄钟时长可配置（低年级建议 15-20 分钟）
- Agent 根据题目总量自动计算需要几个番茄钟
- 每个番茄钟结束时生成"本轮小结"
...
- Android App：作业拍照录入 + 定时采集 + 模糊/遮挡检测 + 番茄钟
- AI Agent：题目解析 + 进度追踪 + 效率分析 + 异常检测
- Web App：实时看板 + 作业报告
